...
func main() {
	var (
		...
		seedUser  = flag.String("seed-user", envOr("CHINASCLMDAV_SEED_USER", "vipiu"), "username to seed on first run")
		seedEmail = flag.String("seed-email", envOr("CHINASCLMDAV_SEED_EMAIL", "vipiu@qq.com"), "email for the seeded user")
		seedName  = flag.String("seed-display", envOr("CHINASCLMDAV_SEED_NAME", "DAV"), "display name for the seeded user")
		seedPass  = flag.String("seed-pass", envOr("CHINASCLMDAV_SEED_PASS", ""), "password for the seeded user (required on first run)")
	)
	flag.Parse()
